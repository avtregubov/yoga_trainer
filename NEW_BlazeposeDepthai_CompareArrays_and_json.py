...
def SravnenieMassivov(ppp, ppp_old):# 1 - равны | 0 - не равны
    if len(ppp) != len(ppp_old):
        return 0
    else:
        for i in range(len(ppp)):
            for j in range(2):
                if abs(ppp[i][j] - ppp_old[i][j]) > 20:
                    return 0
        return 1
...
class BlazeposeDepthai:
    def __init__(self, input_src=None,
        ...
        if self.show_3d:
            ...
            self.vis3d.create_window(window_name='Skeleton', width=1000, height=562)
    ...
    def lm_render(self, frame, region):
        if region.lm_score > self.lm_score_threshold:
            ...
            if self.show_landmarks:
                ...
                #print('region landmarks: ', len(region.landmarks_padded))
                ...
                if self.show_3d:
                    ...
                    ppp = points.tolist() # новый фрейм
                    indicator = SravnenieMassivov(ppp, ppp_old)
                    ppp_old = ppp
                    timestamp = int(time.time())
                    name = 'points_' + str(timestamp) + '.json'
                    if indicator == 1:
                        flag += 1
                        if flag == 3:
                            print("Массивы равны")
    ...
    def run(self):
        ...
        while True:
            ...
            new_window = cv2.resize(annotated_frame, (540, 540))
...
if __name__ == "__main__":
    ...
    parser.add_argument('-3', '--show_3d', action="store_true", default="-3",
